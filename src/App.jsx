...
// ═══════════════════════════════════════════════════════════
// ── PlaceOne: binary insert a single item into an existing ordered list ──
function PlaceOne({newTask, existingTasks, onDone, onCancel}) {
  // State: lo/hi bracket where newTask should be inserted
  const [lo,setLo]=useState(0);
  const [hi,setHi]=useState(existingTasks.length);
  const lockRef=useRef(false);
  const MULTI="linear-gradient(135deg,rgba(230,200,180,0.92) 0%,rgba(210,195,220,0.92) 35%,rgba(190,215,200,0.92) 70%,rgba(220,210,185,0.92) 100%)";

  const choose=(newTaskIsMoreUrgent)=>{
    if(lockRef.current)return;
    lockRef.current=true;
    const mid=Math.floor((lo+hi)/2);
    let newLo=lo, newHi=hi;
    if(newTaskIsMoreUrgent) newHi=mid; else newLo=mid+1;
    if(newLo>=newHi){
      onDone(newLo);
    } else {
      setLo(newLo);
  ...
  // Edge cases — empty list or single item: just call onDone in the handler, not in render
  if(existingTasks.length===0){
    return(
      <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(20,30,10,0.7)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{background:"#fff",borderRadius:24,padding:24,maxWidth:400,width:"100%",textAlign:"center"}}>
          <div style={{fontSize:16,fontWeight:700,marginBottom:16}}>No other items to compare with — adding to top of list.</div>
          <button onClick={()=>onDone(0)} style={{padding:"12px 32px",background:"#5A7848",color:"#fff",border:"none",borderRadius:100,fontWeight:700,fontSize:15,cursor:"pointer"}}>OK</button>
        </div>
      </div>
    );
  }

  // lo >= hi means we found the position
  if(lo>=hi){
    return(
      <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(20,30,10,0.7)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{background:"#fff",borderRadius:24,padding:24,maxWidth:400,width:"100%",textAlign:"center"}}>
          <div style={{fontSize:16,fontWeight:700,marginBottom:8}}>✅ Position found!</div>
          <div style={{fontSize:14,color:"#5A4A30",marginBottom:16}}>"{newTask.name}" will be placed at position {lo+1}</div>
          <button onClick={()=>onDone(lo)} style={{padding:"12px 32px",background:"#5A7848",color:"#fff",border:"none",borderRadius:100,fontWeight:700,fontSize:15,cursor:"pointer"}}>Save position</button>
        </div>
      </div>
    );
  }
  ...
  if(!pivot){
    return(
      <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(20,30,10,0.7)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24}}>
        <div style={{background:"#fff",borderRadius:24,padding:24,maxWidth:400,width:"100%",textAlign:"center"}}>
          <button onClick={()=>onDone(lo)} style={{padding:"12px 32px",background:"#5A7848",color:"#fff",border:"none",borderRadius:100,fontWeight:700,fontSize:15,cursor:"pointer"}}>Add to list</button>
        </div>
      </div>
    );
  }

  const totalSteps=Math.ceil(Math.log2(existingTasks.length+1));
  const stepsDone=totalSteps-Math.ceil(Math.log2(Math.max(1,hi-lo)));
  const pct=Math.min(100,Math.round((stepsDone/Math.max(1,totalSteps))*100));
  ...
  return(
    <div style={{position:"fixed",inset:0,zIndex:500,background:"rgba(20,30,10,0.7)",backdropFilter:"blur(4px)",display:"flex",flexDirection:"column"}}>
      <div style={{background:MULTI,padding:"14px 18px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid rgba(90,80,60,0.10)"}}>
        <button onClick={onCancel} style={{background:"none",border:"none",cursor:"pointer",fontSize:22,padding:0,color:"#1A1A10"}}>✕</button>
        <div style={{fontFamily:"Georgia,serif",fontWeight:700,fontSize:17,color:"#1A1A10",flex:1}}>📍 Where does this fit?</div>
        <div style={{fontSize:11,color:"#5A4A30",fontWeight:700,background:"rgba(255,255,255,0.5)",borderRadius:8,padding:"3px 8px"}}>~{totalSteps-stepsDone} q left</div>
      </div>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px 20px",gap:0,overflowY:"auto"}}>
        <div style={{width:"100%",maxWidth:480}}>
          ...
          <div style={{fontSize:13,fontWeight:700,color:"#1A1A10",textAlign:"center",marginBottom:20,background:"rgba(255,255,255,0.75)",borderRadius:10,padding:"8px 14px"}}>
            Which is more urgent?
          </div>
          <button onClick={()=>choose(true)}
            style={{width:"100%",marginBottom:12,padding:"18px 20px",background:"linear-gradient(135deg,rgba(90,120,72,0.18),rgba(90,120,72,0.06))",border:"2.5px solid rgba(90,120,72,0.40)",borderRadius:20,textAlign:"left",cursor:"pointer",display:"flex",alignItems:"center",gap:14,userSelect:"none",WebkitUserSelect:"none"}}>
            <div style={{width:36,height:36,borderRadius:"50%",background:"#5A7848",color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,flexShrink:0}}>NEW</div>
            <div style={{flex:1,fontWeight:700,fontSize:15,color:"#1A1A10"}}>{newTask.name}</div>
          </button>
          <div style={{textAlign:"center",fontSize:12,fontWeight:700,color:"rgba(90,80,60,0.55)",marginBottom:12}}>OR</div>
          <button onClick={()=>choose(false)}
            style={{width:"100%",padding:"18px 20px",background:MULTI,border:"2px solid rgba(180,160,140,0.30)",borderRadius:20,textAlign:"left",cursor:"pointer",display:"flex",alignItems:"center",gap:14,userSelect:"none",WebkitUserSelect:"none"}}>
            <div style={{width:36,height:36,borderRadius:"50%",background:"rgba(90,80,60,0.12)",color:"#3A2A18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,flexShrink:0}}>#{mid+1}</div>
            <div style={{flex:1,fontWeight:700,fontSize:15,color:"#1A1A10"}}>{pivot.name}</div>
          </button>
          <div style={{marginTop:16,fontSize:12,color:"rgba(255,255,255,0.85)",textAlign:"center",fontWeight:600,textShadow:"0 1px 4px rgba(0,0,0,0.4)"}}>
